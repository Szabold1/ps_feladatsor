...
public class Feladat14 extends TabItem {
	...
	private static final String DATA_URL_NAME = GWT.getHostPageBaseURL() + "adatbazis14.php";
	...
	private ToolBar createToolbarWithBtns() {
		...
		btnDelete.addListener(Events.OnClick, new Listener<ButtonEvent>() {
			@Override
			public void handleEvent(ButtonEvent event) {
				List<ModelData> allSelected = grid.getSelectionModel().getSelectedItems();
				if (allSelected.size() > 0) {
					Map<String, Object> dataMap = new HashMap<String, Object>();

					for (ModelData selected : allSelected) {
						store.remove(selected);
						
						dataMap.put("email", selected.getProperties().get("email"));
						dataMap.put("birthdate", selected.getProperties().get("birthdate"));
						
						sendDataToBackend(dataMap, "DELETE");
					}
				} else {
					MessageBox.alert("Error", "Please select at least one row to delete.", null);
				}
			}
		});
	...
	private Window createWindowWithForm(final ModelData existingData) {
		...
		saveBtn.addListener(Events.OnClick, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent event) {
				...
				sendDataToBackend(dataMap, "POST");
	...
	private void sendDataToBackend(Map<String, Object> dataMap, String requestType) {
		StringBuilder dataToSend = new StringBuilder();

		dataToSend.append("action=").append(URL.encode(requestType));
		...
		final boolean requestIsDelete = requestType.equalsIgnoreCase("DELETE");
			...
			requestBuilder.sendRequest(dataToSend.toString(), new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					if (response.getStatusCode() == 200) {
						if (requestIsDelete)
							Info.display("Success", "Data deleted successfully!");
						else
